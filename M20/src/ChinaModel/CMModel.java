...
//	public Network networkByAge;
	...
	public void start() {		
		...
		parameters = new Parameters(this.job());
		
		//Set random seed
		//this.setSeed(10000 * this.job());	
		//random.setSeed(10000 * this.job());
		...
		//Setup Network
		System.out.println("set Nuclear Family Network...");
		setNuclearFamilyNetwork();
		System.out.println("set Village Network...");
		setVillageNetwork();
		System.out.println("set Province Network...");
		setProvinceNetwork();
		System.out.println("set Migration Network...");
		setMigrantNetwork();
	...
	public void readProvinceData(){
		...
		try { 			
			BufferedReader reader = new BufferedReader(new FileReader("ProvinceWideData.csv"));
			reader.readLine();
			String line = null; 
			...
			while(i < numProvinces && (line=reader.readLine())!=null){ 						
				...
				fdi95 =  Double.parseDouble(item[13]);
				fdi96 =  Double.parseDouble(item[14]);
				fdi97 =  Double.parseDouble(item[15]);
				fdi98 =  Double.parseDouble(item[16]);
				fdi99 =  Double.parseDouble(item[17]);
	...
	public void readIndividualData(){
		try { 
			BufferedReader reader = new BufferedReader(new FileReader("InitialAgentData2Assigned.csv"));
			reader.readLine();
			String line = null; 
			int i = 0;
			int hid = 0;
		...
		for (m = 0; m < numProvinces; m++){
			for (i = 0; i < firm[m].village.numObjs;i++){
				...
				for (j = 0; j < village.numObjs; j++){
					for (k = 0; k < j; k++){
						prob = parameters.c2/Math.pow(village.numObjs, parameters.eta2);
						if( prob > 1)
							prob = 1;
						if (this.random.nextBoolean(prob)){
							...
							I = 1 - (1 - agent1.moved) * (1 - agent2.moved);
							
							if (network.getEdge(agent1, agent2) == null)
								network.addEdge(agent1, agent2, new Double(2));
							if (this.random.nextBoolean(parameters.Cpa ))
							{
				...
				for (j = 0; j < i; j++){
					...
					for (k = 0; k < village1.numObjs; k++)
						for(l = 0;l < village2.numObjs;l++){
							prob = parameters.c3/Math.pow(firm[m].nuclearFamily.numObjs, parameters.eta3);
							if( prob > 1)
								prob = 1;
							if (this.random.nextBoolean(prob)){
								//System.out.println(i);
	...
	public void setMigrantNetwork(){
		...
		for (i = 0; i < this.numProvinces; i++)
		{
			...
			{
				for (k = 0; k < j ; k++){
					prob = parameters.c4/Math.pow((j + 1.0), parameters.eta4);
					if( prob > 1)
						prob = 1;
					if(random.nextBoolean(prob)){
						...
						if (this.random.nextBoolean(parameters.Cpa ))
						{
							edges1 = network.getEdgesIn(agent1);
							edge1 = (Edge)edges1.get(random.nextInt(edges1.numObjs));
							agent3 = (Agent)edge1.getOtherNode(agent1);
							edges2 = network.getEdgesIn(agent2);
							edge2 = (Edge)edges2.get(random.nextInt(edges2.numObjs));
							agent4 = (Agent)edge2.getOtherNode(agent2);
							if (network.getEdge(agent1, agent4) == null)
								network.addEdge(agent1, agent4, new Double(4));
							if (network.getEdge(agent2, agent3) == null)
								network.addEdge(agent2, agent3, new Double(4));
						}
					}
...
*/	
	...
	public int readVillageSize(int provinceIndex){
		...
		double i = 1.0;
		try { 
			BufferedReader reader = new BufferedReader(new FileReader("VillageSizeByProvince.csv"));
			reader.readLine();
			String line = null; 
...
*/
	public static void main(String[] args)
	{
		int jobs = 20; // let’s do 100 runs
		for(int job = 0 ; job < jobs; job++)
		{
			SimState state = new CMModel(System.currentTimeMillis()); 
			state.nameThread();
			...
			break;
			while(state.schedule.getSteps() < 60);
