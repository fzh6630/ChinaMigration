...
//	public int employerID;
	...
	public void marketMatch(CMModel model)
	{
		...
		maxInfluence = model.firm[this.hukou - 1].origins.numObjs * 0.025   - 45;
		maxInfluence = maxInfluence/100;
	
		if (  moved < 1 && ((moveProb ) > 1- (1- threshold) * (1 + maxInfluence)) && isLegalAge()) {//test
		...
		//	randomDest = model.random.nextInt(model.numProvinces);
			if (rational){		
				...
				for (i = 0; i < model.numProvinces ; i++){
					sum += model.firm[i].currentFdi * (1 + (double) model.firm[i].employees.numObjs/(double)sum2);
				...
				for (i = 0; i < model.numProvinces ; i++){
					...
					rand -= model.firm[i].currentFdi * (1 + (double) model.firm[i].employees.numObjs/(double)sum2) /sum;
